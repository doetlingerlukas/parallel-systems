...
So we used *rand_r* with a different seed for each thread, dependent of its *thread_num*.
Each thread as a local copy of its own *seed*, *x* and *y* values. Using the *reduction* possiblity of OpenMP on the *count* variable parallelizing the code is straight forward. (TODO)
