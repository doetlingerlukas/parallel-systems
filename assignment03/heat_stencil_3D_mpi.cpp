...
int main(int argc, char **argv) {
  ...
  for (auto t = 0; t < timesteps; t++) { 
    ...
    for (auto slice = 0; slice < chunk_size; slice++) {
      ...
      if (lower >= 0) {
        MPI_Request req;
        MPI_Isend(&buffer[slice][chunk_size-1][0], chunk_size, MPI_DOUBLE, lower, TO_LOWER, comm_3d, &req);
        MPI_Request_free(&req);
      }
      // send to upper and recieve from upper
      if (upper >= 0) {
        MPI_Request req;
        MPI_Isend(&buffer[slice][0][0], chunk_size, MPI_DOUBLE, upper, TO_UPPER, comm_3d, &req);
        MPI_Request_free(&req);
      ...
      for (auto row = 0; row < chunk_size; row++) { 
        ...
        if (left >= 0) {
          MPI_Request req;
          MPI_Isend(&buffer[slice][row][0], 1, MPI_DOUBLE, left, TO_LEFT, comm_3d, &req);
          MPI_Request_free(&req);
        }
        // send last element of current row to right neigbour
        if (right >= 0) {
          MPI_Request req;
          MPI_Isend(&buffer[slice][row][chunk_size - 1], 1, MPI_DOUBLE, right, TO_RIGHT, comm_3d, &req);
          MPI_Request_free(&req);
        }

        if ((back >= 0) && (slice == chunk_size - 1)) {
          MPI_Request req;
          MPI_Isend(&buffer[slice][row][0], chunk_size, MPI_DOUBLE, back, TO_BACK, comm_3d, &req);
          MPI_Request_free(&req);
        }

        if ((front >= 0) && (slice == 0)) {
          MPI_Request req;
          MPI_Isend(&buffer[slice][row][0], chunk_size, MPI_DOUBLE, front, TO_FRONT, comm_3d, &req);
          MPI_Request_free(&req);
